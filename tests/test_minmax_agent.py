...
def test_minimax_takes_available_box() -> None:
    ...
    # p1 can complete (0,0) with v(0,1)
    decision = agent.select_move(state)

    assert decision.move == ("v", 0, 1)


def test_minimax_does_not_mutate_input_state() -> None:
    state = DotsAndBoxesState(2, 2)
    before = state.clone()
    agent = MinimaxAgent(depth=2, seed=123)
